    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'create':
                ...
                $s3_uri = $_POST['s3_uri'] ?? null;
                $item_arn = $_POST['item_arn'] ?? null;
                $etag = $_POST['etag'] ?? null;
                $stmt = $pdo->prepare("INSERT INTO items (name, description, valid_service, s3_uri, item_arn, etag) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$_POST['name'], $_POST['description'], $valid, $s3_uri, $item_arn, $etag]);
    ...
    <div class="container">
        ...
        <div class="form-section">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>S3 URI (Image URL):</label>
                    <input type="text" name="s3_uri" placeholder="https://bucket.s3.region.amazonaws.com/image.jpg">
                </div>
                <div class="form-group">
                    <label>Item ARN:</label>
                    <input type="text" name="item_arn" placeholder="arn:aws:s3:::bucket/object">
                </div>
                <div class="form-group">
                    <label>ETag:</label>
                    <input type="text" name="etag" placeholder="Object ETag">
                </div>
                <div class="form-group">
                    <label>
        ...
        <?php if (count($items) > 0): ?>
            <table class="items-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <?php if (!empty($item['s3_uri'])): ?>
                                <img src="<?= htmlspecialchars($item['s3_uri']) ?>" alt="Item" class="item-image">
                            <?php else: ?>
                        ...
                        <td style="font-size: 11px;">
                            <?php if (!empty($item['item_arn'])): ?>
                                <strong>ARN:</strong> <?= htmlspecialchars(substr($item['item_arn'], 0, 30)) ?>...<br>
                            <?php endif; ?>
                            <?php if (!empty($item['etag'])): ?>
                                <strong>ETag:</strong> <?= htmlspecialchars(substr($item['etag'], 0, 20)) ?>...
                            <?php endif; ?>
                        </td>
                        <td>
